...
## Fase 21.4: RegisterBusiness first viewport cleanup

Fase 21.4 aplico un slice visual minimo sobre el primer viewport de `RegisterBusiness`. El objetivo fue reducir la sensacion de tarjetas tecnicas/anidadas antes del formulario multi-step, sin cambiar comportamiento ni mover logica.
...
- El bloque superior dejo de estar dentro de `AppCard` y paso a un `section` plano con padding lateral minimo.
- Se redujo el spacing vertical general del `PageShell`.
- La envoltura principal del formulario redujo padding para ganar contenido visible en el primer viewport.
- Las cards de `Paso actual` y `Estado del registro` bajaron peso visual: radios/padding menores, fondo mas sobrio y sin sombra adicional.
- Se actualizaron los snapshots visuales desktop/mobile de `RegisterBusiness`.

Comportamiento preservado:

- No se tocaron formulario, validaciones, submit, upload, geolocalizacion, duplicate precheck, tracking runtime, rutas, auth, API, backend, seed ni contratos.
- No se tocaron handlers, estados, `useEffect`, datos base, ciudades/sectores, revision final ni publicacion.
...
- `git diff --check` puede reportar LF/CRLF en Windows sin bloquear si sale con codigo 0.

Riesgos pendientes:

- El flujo completo de `RegisterBusiness` sigue siendo sensible por formulario multi-step, validaciones, duplicate precheck, upload, geolocalizacion y publicacion.
- No tocar submit, validaciones, geolocalizacion, duplicados, API, tracking ni revision final sin fase dedicada.

Proximo paso recomendado:

- Commit/push de Fase 21.4 como bloque separado.
- Luego iniciar diagnostico visual de `RegisterBusiness` paso/formulario interno o pasar a una vista menos riesgosa si se quiere reducir ruido visual sin tocar flujos de alta friccion.

## Fase 21.5: RegisterBusiness step navigation cleanup

Fase 21.5 aplico un slice visual minimo sobre la navegacion por pasos de `RegisterBusiness`. El objetivo fue reducir el peso de otra tarjeta tecnica dentro del primer flujo visible, manteniendo la navegacion y el progreso intactos.
...
- La tarjeta `Navegacion por pasos` conserva titulo, descripcion, progreso y botones, pero baja padding y elimina sombra local.
- El header interno de la tarjeta queda mas compacto.
- La barra de progreso queda mas delgada, con spacing interno restaurado para evitar que se vea apretada contra los botones.
- Se actualizaron snapshots visuales desktop/mobile de `RegisterBusiness`.

Comportamiento preservado:

- No se tocaron `goToStep`, estados, handlers, validaciones, submit, duplicate precheck, upload, geolocalizacion, tracking runtime, rutas, auth, API, backend, seed ni contratos.
- No se oculto copy ni se cambio el orden funcional de pasos.
...
- La `Revision final` sigue siendo muy dominante visualmente y mezcla guidance, checklist, alertas y acciones sugeridas.
- No tocar `PublicationGuidancePanel`, validaciones, submit, duplicados, geolocalizacion, upload, API ni tracking sin fase dedicada.

Proximo paso recomendado:

- Commit/push de Fase 21.5 como bloque separado.
- Si se sigue con `RegisterBusiness`, disenar primero una fase para simplificar visualmente `PublicationGuidancePanel` sin cambiar reglas de publicacion ni contenido funcional.

## Fase 21.7: RegisterBusiness publication guidance cleanup

Fase 21.7 aplico un slice visual minimo sobre `PublicationGuidancePanel`. El objetivo fue bajar el peso visual de la revision final del registro de negocio sin cambiar reglas de publicacion, contenido funcional ni el flujo multi-step.

Archivos tocados:

- `apps/web/src/pages/register-business/PublicationGuidancePanel.tsx`
- `playwright/specs/__snapshots__/visual.spec.ts/register-business-desktop.png`
- `playwright/specs/__snapshots__/visual.spec.ts/register-business-mobile.png`

Cambios principales:

- El panel de revision final conserva estado, puntaje, alertas, tips, checklist, pendientes y acciones sugeridas, pero usa un fondo menos dominante.
- El header del panel queda mas estable en mobile con layout vertical y la pill de alertas menos pesada.
- El checklist y los bloques de pendientes/acciones bajan profundidad visual sin cambiar orden ni contenido.
- Se actualizaron snapshots visuales desktop/mobile de `RegisterBusiness`.

Comportamiento preservado:

- No se tocaron `businessSubmissionGuidance`, `visibilityChecks`, `remainingPublishNeeds`, `recommendedActions`, puntajes, severidad, reglas de bloqueo ni copy funcional.
- No se tocaron formulario, validaciones, submit, duplicate precheck, upload, geolocalizacion, tracking, rutas, auth, API, backend, seed ni contratos.
...
- El flujo completo de `RegisterBusiness` sigue siendo sensible por formulario multi-step, validaciones, duplicate precheck, upload, geolocalizacion y publicacion.
- No tocar submit, validaciones, geolocalizacion, duplicados, API, tracking ni reglas de revision final sin fase dedicada.
